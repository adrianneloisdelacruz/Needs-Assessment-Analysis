...
<body>
  ...
  $bid = !isset($_REQUEST['bid']) ? "" : htmlspecialchars($_REQUEST['bid']); //proj id
  $tag = !isset($_REQUEST['tag']) ? "" : $_REQUEST['tag']; //
  $proid = !isset($_REQUEST['proid']) ? "" : htmlspecialchars($_REQUEST['proid']);
  ...
  if (mysqli_num_rows($rst2) != 0) {
    ...
    $today = date("d/m/Y", strtotime($row2['reference_date']));
    $comments = $row2['comments'];
  }

  if (isset($_REQUEST['submit'])) {

    $sql = "SELECT * FROM project_evaluation WHERE reco_id='$recoid'";
    ...
    if (mysqli_num_rows($rst) == 0) {
      ...
      $project_title  = "";
      $pid = $_POST['pid'];
      $status = 2; //	0=create,1=ongoing,2=completed
      $today = $_POST['today'];
      ...
      //                VALUES('$name_community','$proid','$college_schoolname','$barangayid','$project_leader','$members',' $personel_involved','$start_date','$end_date','$project_time','$age_bracket','$total_score','$evaluator_name',CURRENT_TIMESTAMP)";
      // $rst = mysqli_query($con, $sql_insert);       
      $sql_insert = "INSERT INTO project_evaluation(community_name,project_id,reco_id,college_school_name,barangay,project_leader,members,personel_involved,project_start_date,project_end_date,project_time,age_bracket,total_score,evaluator_name,comments,status,reference_date,date_created) 
                      VALUES('$name_community','$proid','$recoid','$college_schoolname','$barangayid','$project_leader','$members',' $personel_involved','$start_date','$end_date','$project_time','$age_bracket','$total_score','$evaluator_name','$comments','$status','$today',CURRENT_TIMESTAMP)";
      $rst = mysqli_query($con, $sql_insert);

      if (mysqli_query($con, $sql_insert)) {
        $update_sql = "UPDATE needs_assessement_recommendation SET status='$status' WHERE reco_id='$recoid'";
      ...
      $total_score = $_POST['total_score'];
      $evaluator_name = $_POST['evaluator_name'];
      $status = 2; //	0=create,1=ongoing,2=done
      $today = $_POST['today'];
      $comments = $_POST['comments'];
      ...
      $sql_update = "UPDATE project_evaluation SET community_name='$name_community',college_school_name='$college_schoolname',barangay='$barangayid',project_leader=' $project_leader',members=' $members',
        personel_involved='$personel_involved',project_start_date='$start_date',project_end_date='$end_date',project_time='$project_time ',age_bracket='$age_bracket',
        total_score='$total_score',evaluator_name='$evaluator_name',status='$status',comments='$comments' WHERE reco_id='$recoid'";
      ...
      if (mysqli_query($con, $sql_update)) {
        ...
        $msg_label =  "Project Evaluation record updated successfully!";
        $msg_style = "bi-star";
        $style = "";
      } else {
  ...
  <section class="section">
    <div class="card">
      <div class="card-body">
        <h5 class="card-title">I. Extension Project Title : <?php echo selectProject($proid); ?></h5>

        <!-- Multi Columns Form -->
        <form class="row g-3" method="post" action="main.php?action=projectEvalres">
          <h8 class="card-title">II. Proponent's</h8>
          <div class="col-md-6">
          ...
          <h8 class="card-title">III. Implementing College/Agency/Barangay/Sector</h8>
          <div class="col-md-6">
          ...
          <!--
                ...
                <div class="col-12">
                  ...
                  -->
          <h8 class="card-title">IV. Criteria</h8>
          <div class="col-md-6">
            ...
            <table class="table table-bordered">
              <thead>
                <tr align="center">
                  ...
                  <th scope="col" align="center">RATINGS (%)</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>Contribution to national/regional/local development thrust including impact to beneficiaries/clients. </td>
                  <td align="center">40</td>
                </tr>
                <tr>
                  <td>Suitability and Feasibility. </td>
                  <td align="center">25</td>

                </tr>
                <tr>
                  <td>Resources generation/sourcing </td>
                  <td align="center">25</td>

                </tr>
                <tr>
                  <td>Sectoral focus/potential clients </td>
                  <td align="center">10</td>

                </tr>
                <tr>
                  <th scope="col">Total </td>
                  <th scope="col" align="center">100%</td>
                </tr>
                </tr>
              </tbody>
            </table>
          ...
          <div class="col-md-6">
            <label for="inputEval" class="form-label">
              <h8 class="card-title"> Total Rate: </h8>
            </label>
            <input type="text" class="form-control" name="total_score" value="<?php echo $total_score ?>" required>

          </div>
          <div class="col-md-12">
            <label for="inputEval" class="form-label">
              <h8 class="card-title"> V. Comments/Suggestions </h8>
            </label>
            <textarea class="form-control" style="height: 100px" name="comments" required><?php echo $comments ?></textarea>

          </div>
          <div class="col-md-4">
            <label for="inputEval" class="form-label">Rater/Evaluator:</label>
            <input type="text" class="form-control" id="inputEval" name="evaluator_name" value="<?php echo $evaluator_name ?>" required>
          </div>
          <div class="col-md-4">
            <label for="inputEval" class="form-label">Date:</label>
            <?php $input = ($today != "") ?  "text" : "date"; ?>
            <input type="<?php echo $input; ?>" class="form-control" id="inputEval" name="today" value="<?php echo $today ?>" required>
          </div>
          <!-- <div class="col-md-4">
                  <label for="inputEval" class="form-label">Project Status</label>
                <select class="form-select" aria-label="Default select example" name="status" id="status">  
                  <option value="0" <?php echo $status == 0 ? 'selected' : ''; ?>>Created</option>
      ...
      <div class="text-center">
        ...
        <a href="main.php?action=projectEvaluationList"><button type="button" class="btn btn-warning">Back</button></a>
      </div>
